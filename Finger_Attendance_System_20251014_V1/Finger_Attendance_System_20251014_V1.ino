...
bool getData(const String &device_id, const String &cid, uint8_t retryCount = 2) {
  ...
  if (httpCode == HTTP_CODE_OK) {
    ...
    if (error) {
      ...
      // Basic validation that we got expected response
      if (doc["status"].is<String>()) {
        ...
        if (status == "success") {
          ...
          // Process the actual data received from server
        ...
        // If no status field, assume success and process data
...
// Add this function to process the received data
void processReceivedData(JsonDocument &doc) {
  Serial.println("[DATA] Processing received data...");
  
  // Example: Check if there are user data updates
  if (doc["users"].is<JsonArray>()) {
    ...
    for (JsonObject user : users) {
      if (user["emp_id"].is<String>() && user["name"].is<String>()) {
        ...
        // You could update local user database here
        // updateUserRecord(emp_id, name);
      }
    }
  }

  // Example: Check for system configuration updates
  if (doc["config"].is<JsonObject>()) {
    ...
    if (config["sync_interval"].is<unsigned long>()) {
      ...
      // Update your sync interval here
    }
  ...
  // Example: Check for fingerprint template updates
  if (doc["fingerprints"].is<JsonArray>()) {
    ...
    for (JsonObject fp : fingerprints) {
      if (fp["emp_id"].is<String>() && fp["action"].is<String>()) {
        ...
        // Handle fingerprint updates (add/delete/enable/disable)
        // processFingerprintUpdate(emp_id, action, fp);
      }
    }
  }

  // Example: Check for server messages
  if (doc["message"].is<String>()) {
    String message = doc["message"].as<String>();
    Serial.printf("[DATA] Server message: %s\n", message.c_str());
    
    // Display message on screen or handle accordingly
  ...
  // Example: Check for attendance data
  ...
  // Log any additional fields for debugging
...
void loop() {
  maintainWiFi();
  sendNewFingerprintsToServer(); // Add this line
...
void enrollFingerprint(int id) {
    // Get name (initially blank, will be set by server)
  String name = ""; // Blank name initially
  // ID Validation
  if (id == -1) {
...
bool saveFingerprintTemplate(int id, const String &name) {
  String templateData = getFingerprintTemplateAsString(id);
  ...
  // Save to pending file: id,timestamp,template_data,name
  String record = String(id) + "," + timestamp + "," + templateData + "," + name;
  return appendToSD(PENDING_FINGERPRINTS_FILE, record + "\n");
}

/**
// Add this function to save fingerprint data
bool saveFingerprintTemplate(int id, const String &name) {
  String templateData = getFingerprintTemplateAsString(id);
  if (templateData.isEmpty()) {
    Serial.println("Failed to get fingerprint template");
...
*/
/**
// Function to get fingerprint template as string (hex format)
String getFingerprintTemplateAsString(int id) {
  if (finger.loadModel(id) != FINGERPRINT_OK) {
  ...
  // Get template data
  uint8_t templateBuffer[512]; // Adjust size based on your sensor
  int templateSize = finger.downloadModel(templateBuffer);
  ...
  for (int i = 0; i < templateSize; i++) {
    if (templateBuffer[i] < 16) hexString += "0";
...
*/
// Replace the getFingerprintTemplateAsString function with this:
String getFingerprintTemplateAsString(int id) {
  // For most fingerprint sensors, we can't directly access the raw template data
  // Instead, we'll create a unique identifier based on the fingerprint ID and timestamp
  // You may need to check your specific sensor's documentation for template extraction
  
  DateTime now = rtc.now();
  String uniqueId = String(id) + "_" + 
                   String(now.year()) + 
                   String(now.month()) + 
                   String(now.day()) + 
                   String(now.hour()) + 
  ...
  // If your sensor supports template extraction, use that instead
  // For now, we'll use this unique identifier
  // Note: You'll need to implement actual template extraction based on your sensor model
  
  Serial.printf("Generated fingerprint ID for ID %d: %s\n", id, uniqueId.c_str());
  return uniqueId;
}
...
bool sendNewFingerprintsToServer() {
  ...
  // Load pending fingerprints
  String pendingData = readFromSD(PENDING_FINGERPRINTS_FILE);
  if (pendingData.isEmpty()) {
    return true; // No pending records is success
  }
  ...
  JsonArray records = doc["new_fid_records"].to<JsonArray>();
  ...
  while (startPos < pendingData.length() && recordCount < MAX_FINGERPRINT_RECORDS) {
    ...
    if (line.length() > 0) {
      // Parse: id,timestamp,template_data,name
      ...
      if (comma1 != -1 && comma2 != -1 && comma3 != -1) {
        ...
        String fid = line.substring(comma2 + 1, comma3);
        // We don't need the name field for the server payload

        // Add to JSON array (without name field)
        JsonObject record = records.add<JsonObject>();
        record["timestamp"] = timestamp;
        record["emp_id"] = emp_id;
        record["fid"] = fid;
        // Note: No "name" field added
        
        recordsProcessed++;
        processedRecords[recordCount] = line; // Store for cleanup
        recordCount++;
      }
    }
    startPos = endPos + 1;
  ...
  Serial.printf("Attempting to sync %d fingerprint records\n", recordsProcessed);

  // Send to server
  HTTPClient http;
  String url = base_url + "/attendify/api/send_new_fid";
  http.begin(url);
  http.addHeader("Content-Type", "application/json");
  http.setTimeout(15000);
  ...
  if (httpCode == HTTP_CODE_OK) {
    success = true;
    Serial.println("Fingerprint records synced successfully");
    
    // Remove successfully sent records from pending file
    ...
    if (getToken(device_id, default_token)) {
      http.end();
      return sendNewFingerprintsToServer(); // Retry
    }
  } else {
    String response = http.getString();
...
// Replace the removeSentFingerprintRecords function with this:
void removeSentFingerprintRecords(String sentRecords[], int recordCount) {
  ...
  // Save remaining records or delete file if empty
