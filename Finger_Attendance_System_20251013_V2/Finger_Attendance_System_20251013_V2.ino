...
void enrollFingerprint(int id = -1) {
  ...
  // Use empty name for new registrations
  String empName = ""; // Empty name - will be set in database
  ...
  Serial.printf("[ENROLL] Starting enrollment for ID: %d (Name will be set in database)\n", id);
  ...
  if (finger.fingerSearch() == FINGERPRINT_OK) {
    ...
    Serial.printf("❌ Fingerprint already registered as ID: %d\n", finger.fingerID);
    return;
  }
  ...
  if (finger.storeModel(id) == FINGERPRINT_OK) {
    ...
    // Send to server with empty name
    if (sendNewFingerprintToServer(id, empName)) {
      successMessage("Registered ID: " + String(id));
    } else {
      // If immediate send fails, save to queue for later sync
      if (saveNewFingerprintToQueue(id, empName)) {
        successMessage("Stored - Sync later");
      } else {
        successMessage("Stored but sync failed");
      }
    }
  } else {
    failMessage("Storage failed");
...
bool saveNewFingerprintToQueue(int id, const String &empName) {
  Serial.println("\n[FID-QUEUE] === Saving New Fingerprint to Sync Queue ===");
  ...
  if (templateData.isEmpty()) {
    Serial.println("❌ [FID-QUEUE] Failed to get fingerprint template");
    return false;
  }

  // Validate template before saving
  if (!isValidTemplate(templateData)) {
    Serial.println("❌ [FID-QUEUE] Invalid fingerprint template");
    return false;
  }
  
  // Save with empty name field
  String record = String(timestamp) + "," + String(id) + ",," + templateData;
  bool success = appendToSD(NEW_FINGERPRINT_FILE, record + "\n");
  
  if (success) {
    Serial.println("✅ [FID-QUEUE] Fingerprint saved to sync queue");
    Serial.print("📁 File: ");
    Serial.println(NEW_FINGERPRINT_FILE);
    Serial.print("🆔 Fingerprint ID: ");
    Serial.println(id);
    Serial.print("📝 Record: ");
    Serial.println(record.substring(0, 50) + "..."); // Show first 50 chars
    
    // Count queued records
    int queuedCount = countQueuedFingerprints();
    Serial.print("📊 Total queued fingerprints: ");
    Serial.println(queuedCount);
  } else {
    Serial.println("❌ [FID-QUEUE] Failed to save fingerprint to queue");
  }
  
  Serial.println("[FID-QUEUE] === End Save Attempt ===\n");
  return success;
}

void processNewFingerprints() {
  static unsigned long lastSyncAttempt = 0;
  static unsigned long retryDelay = SYNC_RETRY_DELAY;

  if (millis() - lastSyncAttempt < retryDelay) return;
  lastSyncAttempt = millis();

  if (WiFi.status() != WL_CONNECTED) {
    Serial.println("📶 [FID-SYNC] Skipping - WiFi disconnected");
    retryDelay = SYNC_RETRY_DELAY;
    return;
  }
  ...
  Serial.printf("\n[FID-SYNC] === Processing %d queued fingerprints ===\n", queuedCount);
  
  // Read all queued fingerprints
  String pendingData = readFromSD(NEW_FINGERPRINT_FILE);
  if (pendingData.isEmpty()) {
    Serial.println("❌ [FID-SYNC] No data to sync");
    return;
  }
  ...
  while (startPos < pendingData.length()) {
    ...
    if (line.length() > 0) {
      ...
      if (comma1 != -1 && comma2 != -1 && comma3 != -1) {
        ...
        String emp_id = line.substring(comma1 + 1, comma2);
        // Name field is between comma2 and comma3, but we ignore it (empty)
        String fingerprint_template = line.substring(comma3 + 1);

        JsonObject record = records.add<JsonObject>();
        record["timestamp"] = timestamp;
        record["emp_id"] = emp_id;
        record["fid"] = fingerprint_template;
        // No name field sent - it will be empty in database
        recordsProcessed++;
        
        Serial.printf("📦 [FID-SYNC] Queued: ID %s, Time: %s\n", 
                     emp_id.c_str(), timestamp.c_str());
      }
    }
    startPos = endPos + 1;
  ...
  if (recordsProcessed == 0) {
    Serial.println("❌ [FID-SYNC] No valid records to process");
    return;
  }

  Serial.printf("🔄 [FID-SYNC] Attempting to sync %d fingerprints\n", recordsProcessed);
  bool success = sendNewFingerprints(doc);

  if (success) {
    Serial.printf("✅ [FID-SYNC] Successfully synced %d fingerprints\n", recordsProcessed);
    Serial.println("💡 Names can be assigned in the database for these IDs");
    deleteFromSD(NEW_FINGERPRINT_FILE);
    retryDelay = SYNC_INTERVAL;
  } else {
    Serial.println("❌ [FID-SYNC] Sync failed - will retry later");
    retryDelay = SYNC_RETRY_DELAY;
  }
  
  Serial.println("[FID-SYNC] === End Sync Cycle ===\n");
}
